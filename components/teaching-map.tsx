...
export default function TeachingMap({initialCompany, initialHub}: {initialCompany: string; initialHub: string}) {
  ...
  const references = companyReferences[company.id] ?? [];
  ...
  useEffect(() => {
    ...
    const siblings = Array.from(shellRef.current?.children ?? []).filter(el => el !== panel) as HTMLElement[];
    siblings.forEach(el => { el.inert = true; });
    panel?.focus();
    const key = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { event.preventDefault(); setSheet(false); }
      if (event.key === 'Tab' && panel) {
        const items = Array.from(panel.querySelectorAll<HTMLElement>('button, select, a[href], input')).filter(el => !el.hasAttribute('disabled'));
    ...
    return () => {siblings.forEach(el => {el.inert = false;});document.removeEventListener('keydown',key);previous?.focus();};
  }, [sheet]);
  ...
  async function saveImage(reveal: boolean) {
    ...
    try {
      ...
      ctx.fillStyle = '#142c51'; ctx.font = `bold 52px ${font}`; ctx.fillText(`${company.name} · 공간적 분업`,80,100);
      ctx.font = `30px ${font}`; ctx.fillText(hub.name,80,155);
      ...
      ctx.fillStyle = '#1d4ed8'; ctx.font = `bold 34px ${font}`;
      let y = wrap(reveal ? `기능: ${hub.typeLabel}` : '어떤 기업 활동에 유리할까요?',1680,285,640);
      hub.reasons.forEach((reason,i) => {ctx.fillStyle='#142c51'; ctx.font=`bold 28px ${font}`; y=wrap(`${i+1}. ${reason.title}`,1680,y+24,640,38); if(reveal){ctx.font=`25px ${font}`; ctx.fillStyle='#425673'; y=wrap(reason.detail,1680,y+6,640,35);}});
      ctx.fillStyle='#425673';ctx.font=`25px ${font}`;ctx.fillText('Natural Earth',80,1220);
  ...
  return <main ref={shellRef} className={`teach-shell ${presenting?'teach-presenting':''} ${sheet?'sheet-open':''}`}>
    <header className="teach-header"><div><Globe2/><strong>GLOBAL SHIFT</strong><span>공간적 분업</span></div><nav aria-label="수업 도구"><details className="teach-ppt"><summary><Download size={16}/> PPT 자료</summary><div className="teach-ppt-panel"><strong>현재 거점을 PPT로</strong><p>이미지를 넣고 장면 링크를 연결하면 같은 거점의 질문 화면이 열립니다.</p><Button variant="outline" disabled={exporting} onClick={()=>saveImage(false)}><Download/>질문용 이미지</Button><Button variant="outline" disabled={exporting} onClick={()=>saveImage(true)}><Download/>설명용 이미지</Button><Button variant="outline" onClick={copyLink}><Link2/>장면 링크 복사</Button></div></details><Button variant="outline" onClick={()=>setSheet(true)}><Printer/>활동지</Button><Button onClick={fullscreen}>{presenting?<Minimize2/>:<Maximize2/>}{presenting?'발표 종료':'수업 화면'}</Button></nav></header>
    <div className="teach-select"><label>기업 <select value={company.id} onChange={e=>select(e.target.value,'')}>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><span className="teach-scope">수업용 주요 거점 {company.hubs.length}곳</span><div className="teach-hubs" aria-label="거점 선택">{company.hubs.map((h,i)=><button key={h.id} aria-pressed={h.id===hub.id} onClick={()=>select(company.id,h.id)}><span>{String(i+1).padStart(2,'0')}</span>{shortName(h.name)}</button>)}</div></div>
    <div className="teach-workspace"><section className="teach-map"><div className="teach-map-title"><span>{company.name}의 글로벌 거점</span><h1>왜 이곳에 자리 잡았을까?</h1></div>
      <svg ref={svgRef} viewBox="0 0 1000 500" role="img" aria-label={`${company.name}의 본사와 거점, 국가별 경계가 표시된 세계 지도`} style={{width:'100%',height:'auto',display:'block',background:'#f0f6ff'}}>
        ...
        {countries.map((c,i)=><path key={i} d={path(c as never)??''} fill="#d4e2f5" stroke="#738eaf" strokeWidth=".65"/>)}
      ...
      {(()=>{const [x,y]=projection([hub.lng,hub.lat])!; const label=shortName(hub.name); const width=Math.min(350,label.length*13+26);const left=Math.max(8,Math.min(992-width,x-width/2));const top=Math.min(465,y+25);return <g aria-hidden="true"><rect x={left} y={top} width={width} height="32" rx="6" fill="white" stroke="#2563eb" strokeWidth="1.5"/><text x={left+width/2} y={top+21} textAnchor="middle" fontFamily="sans-serif" fontSize="14" fontWeight="bold" fill="#153361">{label}</text></g>;})()}
      </svg><div className="teach-map-caption"><strong>본사 · {company.headquarters.city}</strong><span>선택 거점 · {hub.name}</span></div><p className="teach-source">Natural Earth</p>

    </section><aside className="teach-inquiry"><div className="teach-inquiry-scroll"><div className="teach-region">{hub.country}</div><h2>{hub.name}</h2><div className="evidence-status"><strong>{evidence.status}</strong><span>자료 확인 {evidence.checked}</span></div>{evidence.note&&<p className="evidence-note">{evidence.note}</p>}{stage>=1&&<div className="teach-answer" aria-live="polite"><span>확인된 대표 기능</span><strong>{hub.typeLabel}</strong><small>한 거점에서 여러 기능을 함께 수행할 수 있습니다.</small></div>}<div className="teach-steps" aria-label="공개 단계">{steps.map((s,i)=><span key={s} aria-current={i===stage?'step':undefined}>{i+1} {s}</span>)}</div><h3>{stage===0?'어떤 기능에 유리할까요?':stage===1?'어떤 지역 단서가 이 기능과 연결될까요?':'지역 조건은 기업에 어떤 이점을 줄까요?'}</h3><div className="teach-clues">{hub.reasons.map((r,i)=><article key={r.title}><span>추론 단서 {i+1}</span><strong>{r.title}</strong>{stage===2&&<p><em>입지 해석</em>{r.detail}</p>}</article>)}</div>{stage===2&&<><section className="compare-prompt"><span>비교 발문</span><strong>{shortName(hub.name)} ↔ {shortName(second.name)}: 두 거점의 기능과 입지 이유는 어떻게 다를까요?</strong></section><details className="perspective-prompt"><summary>지역과 사람의 관점 · 선택 질문</summary><p>이 거점은 지역의 일자리와 산업에 어떤 변화를 만들까요?</p><p>정부·기업·노동자·주민은 어떤 점을 서로 다르게 볼까요?</p></details><div className="source-links"><span>공식 참고자료</span>{references.map(source=><a key={source.url} href={source.url} target="_blank" rel="noreferrer">{source.label}</a>)}</div></>}</div><div className="teach-controls">{stage<2?<Button onClick={()=>setStage(stage+1)}>{stage===0?'기능 공개':'입지 이유 공개'}<ArrowRight/></Button>:<p>비교 발문으로 두 거점을 연결해 보세요.</p>}<Button variant="outline" disabled={stage===0} onClick={()=>setStage(0)}><RotateCcw/>다시 가리기</Button></div></aside></div>
    ...
    {sheet&&<section ref={sheetRef} className="teach-sheet" role="dialog" aria-modal="true" aria-label="인쇄용 활동지" tabIndex={-1}><div className="sheet-toolbar"><label>비교할 두 번째 거점 <select value={second.id} onChange={e=>setSecondId(e.target.value)}>{company.hubs.filter(h=>h.id!==hub.id).map(h=><option key={h.id} value={h.id}>{h.name}</option>)}</select></label><Button onClick={()=>window.print()}><Printer/>인쇄 / PDF 저장</Button><Button variant="outline" onClick={()=>setSheet(false)}>지도로 돌아가기</Button></div><div className="sheet-paper"><span>공간적 분업 탐구 · {company.name}</span><h1>왜 이곳에서 이 일을 할까?</h1><p className="sheet-name">학년 / 반 __________ 번호 ______ 이름 ______________</p><p>기능을 공개하기 전에 ①~②를 먼저 작성하세요. 예상과 사례가 달라도 괜찮습니다.</p>{[hub,second].map((h,i)=><article key={h.id}><h2>{i+1}. {h.name}</h2><ul className="sheet-clues">{h.reasons.map(r=><li key={r.title}>{r.title}</li>)}</ul><p>① 예상한 기능 (복수 선택 가능): □ 조립·생산 □ 연구개발 □ 부품 생산 □ 자원 채굴 □ 판매·물류</p><div className="sheet-lines">② 이 지역은 ____________________하므로, 기업이 ____________________하는 데 유리하다.</div><div className="sheet-lines">③ 사례 확인 후 실제 기능: ____________________　더 고려할 단서: ____________________</div></article>)}<h2>두 지역을 비교해 한 문장으로 정리해 봅시다.</h2><div className="sheet-lines tall">두 지역은 ____________________라는 점에서 같거나 다르다. 그 이유는</div><p className="sheet-foot">국가의 발전 수준만으로 기업 기능을 단정하지 말고, 지역 조건과 기업 활동의 이점을 연결해 보세요.</p></div></section>}
  </main>;
}
