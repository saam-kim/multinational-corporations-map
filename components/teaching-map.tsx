...
export default function TeachingMap({initialCompany, initialHub}: {initialCompany: string; initialHub: string}) {
  ...
  return <main ref={shellRef} className={`teach-shell ${presenting?'teach-presenting':''} ${sheet?'sheet-open':''}`}>
    ...
    </section><aside className="teach-inquiry"><div className="teach-inquiry-scroll"><div className="teach-region">{hub.country}</div><h2>{hub.name}</h2>{stage>=1&&<div className="teach-answer" aria-live="polite"><span>이 거점의 기능</span><strong>{hub.typeLabel}</strong></div>}<div className="teach-steps" aria-label="공개 단계">{steps.map((s,i)=><span key={s} aria-current={i===stage?'step':undefined}>{i+1} {s}</span>)}</div><h3>{stage===0?'어떤 기능에 유리할까요?':stage===1?'어떤 지역 단서가 이 기능과 연결될까요?':'지역 조건은 기업에 어떤 이점을 줄까요?'}</h3><div className="teach-clues">{hub.reasons.map((r,i)=><article key={r.title}><span>단서 {i+1}</span><strong>{r.title}</strong>{stage===2&&<p>{r.detail}</p>}</article>)}</div></div><div className="teach-controls">{stage<2?<Button onClick={()=>setStage(stage+1)}>{stage===0?'기능 공개':'입지 이유 공개'}<ArrowRight/></Button>:<p>다른 거점과 입지 이유를 비교해 보세요.</p>}<Button variant="outline" disabled={stage===0} onClick={()=>setStage(0)}><RotateCcw/>다시 가리기</Button></div></aside></div>
