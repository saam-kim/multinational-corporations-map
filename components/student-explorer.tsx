...
export default function StudentExplorer() {
  ...
  return (
    <main className={`learning-shell ${presentationMode ? 'presentation-mode' : ''}`}>
      <a href="/" style={{display:'block',padding:'10px 24px',background:'#eaf1ff',color:'#1d4ed8',fontSize:14}}>← 교사 설명·발문용 지도로</a>
      ...
      <div className="lesson-brief"><div><strong>공간적 분업 탐구 · 15~20분</strong><span>두 거점을 골라 기능과 입지 이유를 비교하고, 한 문장으로 정리해요.</span></div><Button variant="outline" onClick={() => setNotebookOpen(true)}>비교 한 문장 · {Object.values(notebook.records).filter(r => r.evidenceOpen).length}곳</Button></div>
      ...
      <section className="workspace">
        ...
        <section className="map-stage" aria-label={`${company.name} 글로벌 거점 지도`}>
          ...
          <div className="map-canvas"><div className="map-grid" />
            ...
            <div className="map-source">NATURAL EARTH</div>
