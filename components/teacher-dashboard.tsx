...
import { type LearningRecord, type Comparison, roleNames } from '@/lib/learning';

type Participant = { records?: Record<string, LearningRecord>; comparison?: Comparison; helpRequested?: boolean; id: string; name: string; companyId?: string; hubId?: string; roleGuess?: string; roleCorrect: number; inference?: string; evidenceOpen: number; quizScore: number; feedback?: string; joinedAt: number; lastSeen: number };
...
export default function TeacherDashboard({ code }: { code: string }) {
  ...
  const needsHelp = (item: Participant) => Boolean(item.helpRequested || Object.values(item.records ?? {}).some(record => record.helpRequested));
  const activeCount = data?.participants.filter(isRecent).length ?? 0;
  const roleCount = data?.participants.filter((item) => Object.values(item.records ?? {}).some(r => r.evidenceOpen) || item.evidenceOpen).length ?? 0;
  const evidenceCount = data?.participants.filter((item) => item.comparison?.first && item.comparison?.second && item.comparison?.explanation.trim()).length ?? 0;
  const helpCount = data?.participants.filter(needsHelp).length ?? 0;
  const filteredParticipants = (data?.participants ?? []).filter((item) => filter === 'all' || (filter === 'help' && needsHelp(item)) || (filter === 'working' && !item.comparison?.explanation.trim()) || (filter === 'done' && Boolean(item.comparison?.first && item.comparison?.second && item.comparison?.explanation.trim())));
  ...
  return <main className="teacher-shell">
    ...
    <section className="teacher-layout">
      ...
      <section className="dashboard-main">
        ...
        <div className="metric-grid"><article><Users /><div><strong>{data?.participants.length ?? 0}</strong><span>입장 학생</span></div></article><article><Radio /><div><strong>{data.session.status === 'paused' ? '—' : activeCount}</strong><span>{data.session.status === 'paused' ? '활동 일시정지 중' : '최근 90초 활동'}</span></div></article><article><Check /><div><strong>{roleCount}</strong><span>가설·이유 제출</span></div></article><article><MessageSquareText /><div><strong>{evidenceCount}</strong><span>비교 정리 작성</span></div></article></div>

        <details className="lesson-guide teacher-lesson"><summary>수업 활용 안내 · 관찰 기준</summary><p>공간적 분업과 입지 요인을 간단히 설명한 뒤, 수업 목표에 맞는 두 거점을 선택하세요. 예: 삼성 베트남·인도 노이다의 생산 기능 비교.</p><p>관찰 기준: ① 지역 단서를 골랐는가 ② 기업 활동의 이점과 연결했는가 ③ 두 지역의 공통점이나 차이점을 설명했는가. 제출·열람은 이해도 점수가 아닙니다.</p><p>기본 활동 15~20분: 두 거점 탐구 → 비교 한 문장 → 함께 읽기. 시간이 남으면 미국 텍사스의 생산 기능이나 인도 벵갈루루의 연구개발을 살펴보세요. 지역의 고용·환경 변화는 후속 수업에서 선택적으로 다룰 수 있습니다.</p></details>
        ...
        <div className="student-filters" aria-label="학생 진행 상태 필터"><div><strong>학생별 진행</strong><span>도움 요청 {helpCount}명</span></div><div>{([['all','전체'],['help','도움 요청'],['working','진행 중'],['done','비교 작성']] as const).map(([key,label]) => <button key={key} className={filter === key ? 'active' : ''} onClick={() => setFilter(key)}>{label}{key === 'help' && helpCount > 0 ? ` ${helpCount}` : ''}</button>)}</div></div>
        <div className="student-board"><div className="board-header"><span>학생</span><span>현재 활동</span><span>추론 내용</span><span>관리</span></div>{filteredParticipants.length ? filteredParticipants.map((student) => { const company = companies.find((item) => item.id === student.companyId); const hub = company?.hubs.find((item) => item.id === student.hubId); return <article key={student.id} className={`student-row ${needsHelp(student) ? 'needs-help' : ''}`}><div className="student-identity"><strong>{student.name}</strong><span><Clock3 /> {activityText(student)}</span>{needsHelp(student) && <em>학생 도움 요청</em>}</div><div className="student-progress"><strong>{company?.name ?? '탐색 전'}</strong><span>{hub?.name ?? '거점 선택 전'}</span><i>{student.evidenceOpen ? '사례 열람' : student.roleGuess ? '가설·이유 작성 중' : '단서 확인 중'}</i></div><p className="student-inference">{student.inference || '아직 작성하지 않았습니다.'}</p><div className="student-action"><Button variant="outline" onClick={() => setSelectedStudentId(student.id)}><MessageSquareText /> 피드백</Button>{student.feedback && <small>전달한 피드백 있음</small>}</div></article>; }) : <div className="empty-class"><Users /><strong>{data?.participants.length ? '이 조건에 해당하는 학생이 없습니다' : '아직 입장한 학생이 없습니다'}</strong><p>{data?.participants.length ? '다른 필터를 선택해 보세요.' : '학생들이 QR 코드를 스캔하면 이곳에 바로 표시됩니다.'}</p></div>}</div>
      </section>
    </section>
    <Dialog open={Boolean(selectedStudent)} onOpenChange={(open) => { if (!open) setSelectedStudentId(''); }}><DialogContent className="feedback-dialog"><DialogHeader><DialogDescription>개별 피드백</DialogDescription><DialogTitle>{selectedStudent?.name} 학생의 추론</DialogTitle></DialogHeader>{selectedStudent && <><div className="feedback-context"><span>{selectedStudent.evidenceOpen ? '가설 제출 후 사례 열람' : '가설 작성 중'}</span><p>{selectedStudent.inference || '아직 이유를 작성하지 않았습니다.'}</p></div><div className="teacher-record-history"><h3>거점별 첫 생각과 보완</h3>{Object.values(selectedStudent.records ?? {}).map(r => <article key={r.hubId}><strong>{companies.find(c => c.id === r.companyId)?.hubs.find(h => h.id === r.hubId)?.name}</strong><p>가설: {roleNames[r.roleGuess ?? ''] ?? '선택 전'} · {r.inference || '작성 전'}</p>{r.revision && <p>보완: {r.revision}</p>}</article>)}{selectedStudent.comparison && <article><strong>두 거점 비교 정리</strong><p>{selectedStudent.comparison.explanation || '아직 작성하지 않음'}</p></article>}</div><label htmlFor="teacher-feedback">학생에게 보낼 피드백</label><textarea id="teacher-feedback" value={feedbackDrafts[selectedStudent.id] ?? selectedStudent.feedback ?? ''} onChange={(event) => { setFeedbackSent((value) => ({ ...value, [selectedStudent.id]: false })); setFeedbackDrafts((value) => ({ ...value, [selectedStudent.id]: event.target.value })); }} placeholder="잘한 점과 다음에 생각해 볼 질문을 짧게 적어 주세요." rows={4} /><DialogFooter><Button variant="outline" onClick={() => setSelectedStudentId('')}>닫기</Button><Button disabled={busy} onClick={async () => { const success = await action({ action: 'feedback', participantId: selectedStudent.id, feedback: feedbackDrafts[selectedStudent.id] ?? selectedStudent.feedback ?? '' }, `${selectedStudent.name} 학생에게 피드백을 보냈습니다.`); if (success) setSelectedStudentId(''); }}><Send /> {feedbackSent[selectedStudent.id] ? '다시 보내기' : '피드백 보내기'}</Button></DialogFooter></>}</DialogContent></Dialog>
