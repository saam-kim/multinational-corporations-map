...
import { comparisonHub, connectionConcept, connectionQuestion } from '../lib/teaching-flow.ts';
...
test('each company prefers a different function when one exists', () => {
  for (const company of companies) {
    for (const hub of company.hubs) {
      ...
      if (company.hubs.some(h => h.type !== hub.type)) assert.notEqual(second.type, hub.type);
    }
  }
});

test('a teacher can explicitly compare the same function', () => {
  assert.equal(comparisonHub(samsung.hubs, vietnam, 'samsung-in-mfg').id, 'samsung-in-mfg');
});

test('return comparison retains an already revealed partner', () => {
  assert.equal(comparisonHub(samsung.hubs, indiaResearch, '', { [vietnam.id]: 2 }).id, vietnam.id);
});
...
test('invalid or current preferences cannot compare a hub with itself', () => {
  for (const preferred of ['missing', vietnam.id, 'apple-tw-tsmc']) {
    assert.equal(comparisonHub(samsung.hubs, vietnam, preferred).id, indiaResearch.id);
  }
...
test('all public scene links render question-first without revealed answers', { skip: !base }, async () => {
  for (const company of companies) {
    await Promise.all(company.hubs.map(async hub => {
      ...
      assert.doesNotMatch(html, /사례의 대표 기능|공간적 분업 연결 정리|자료 확인 2026/);
